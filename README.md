...
> 执行入口:[AGENTS.md](AGENTS.md)(agent 默认读取,所有文档均可从该文件到达)。
...
```
├── AGENTS.md                     # agent 入口:角色、铁律(总命令)、问题协议(不写具体工具)
├── workflows/subtitle-translation/
│   ├── workflow.md               # 流程总纲:阶段总览、入参清单、目录分层、产物命名
│   ├── tools.md                  # 本工作流工具清单:全部工具与完整用法(唯一权威)
...
2. **总 AGENTS.md 只写原则**:总 agent 文档只含工具总命令("有工具必须用工具,没有就反馈")与通用铁律,**不写具体工具**;每个工作流的元指令(tools.md)全面描述该工作流所有工具及用法。
3. **脚手架是工具**:确定性步骤(download/whisper/ffmpeg/srt 结构/报告生成)由脚本保证,agent 不重造轮子、不写重复样板。
4. **判断力属于模型**:源整理、翻译、记忆维护由 agent 完成,不设 JSON 硬约束,靠质量门槛自检兜底。
