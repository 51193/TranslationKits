...
## 目标
从视频抽取音频,whisper 转写为句级时间戳字幕,并导出纯文本。

## 前置
- 01 通过;`video.*` 与 `info.txt` 存在。

## 步骤

1. 抽取音频(照抄本命令,不要改写):
   ```bash
   ffmpeg -y -i <workspace>/video.<ext> -vn -acodec pcm_s16le -ar 44100 <workspace>/audio.wav
   ```
2. 转写(语言/模型取 params):
   ```bash
   tools/transcribe.py <workspace>/audio.wav --output <workspace>/subtitle.srt \
...
3. 导出纯文本:
   ```bash
   tools/srt_tool.py to-txt <workspace>/subtitle.srt <workspace>/raw_subtitle.txt
   ```
4. 结构校验:
   ```bash
   tools/srt_tool.py validate <workspace>/subtitle.srt
   ```
5. 行数比对:`wc -l raw_subtitle.txt` 应等于 validate 输出的条目数。
...
| 现象 | 处置(照表执行) |
|------|------------------|
| ffmpeg 失败 | 检查 `video.*` 是否存在且非空;损坏则重新 fetch(需用户同意);ffmpeg 本身错误 → 走问题协议 |
...
> 禁止:自行换转写引擎、自行安装额外依赖、自行调 whisper 参数做实验。
