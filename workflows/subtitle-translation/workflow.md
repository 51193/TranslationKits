...
凡是"模型语义判断能直接完成"的环节不单独设阶段,由 agent 在语义性阶段内直接完成;确定性环节才设独立阶段或工具:

- **不设独立"规范化"阶段**:合句(合并语法断裂的碎片)是语义判断,在 03 翻译阶段的源整理子步骤直接完成(合并执行交给 [merge 工具](tools.md#merge));工具负责结构,agent 负责判断。
- **不设独立"校对"阶段**:译文由 agent 直接写文件,不会产生空行;空文本由 [validate](tools.md#validate) 检查兜底,重复由翻译自检(quality.md 自检清单)在 04 复核阶段再次全文把关。
- **不保留纯文本视图中间文件**(raw_*.txt):agent 直接读写 SRT 结构,纯文本视图只在必要时由 [to-txt](tools.md#to-txt) 现场生成。
- **确定性操作全部工具化**:抓取(fetch)、转写(transcribe)、合并/拆块/拼块(merge/split/compose)、烧录(burn)、报告(report)由工具保证结构正确,agent 不手算、不手写。
...
- **禁止自造环节脚本**:合并、拆块、拼块、格式转换、结构检查一律用 [srt_tool.py](tools.md#srt-tool) 既有子命令;任何"发现工具缺能力"的情况 → 走问题协议反馈建议,不得为流程环节编写一次性脚本(自造合并脚本易导致序号位移等连环错误)。文本级小修正(如 03 拼接处标点、04 复核修正译文)直接用 Edit 工具编辑文件,不写脚本。
